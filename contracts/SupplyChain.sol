...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint _sku) {
    require (items[_sku].state == State.ForSale, "State needs to be set to 0 for sale");
    require (items[_sku].seller != address(0), "Item needs to be initialised");
    _;
  }
  modifier sold(uint _sku) {
    require (items[_sku].state == State.Sold, "State needs to be Sold ('1')");
    _;
  }
  modifier shipped(uint _sku) {
    require (items[_sku].state == State.Shipped, "State needs to be Shipped ('2')");
    _;
  }
  modifier received(uint _sku) {
    require (items[_sku].state == State.Received, "State needs to be Received");
    _;
  }
